...
(function () {
  ...
  /* ملاحظة هامة:
     ...
     (db.orders، المتزامن كل 6 ثواني) — هل لسه فيه طرف من نفس سلسلة
     التنفيذ الجزئي (rootOrderId) "pending" (مفيش صيدلية اتعينت له
     لسه)، ولو أه بترجع { blocked: true, reason: "pending" } عشان
     الفرونت إند (pages1.js) يعطّل زرار "خرج للتوصيل" ويوضح السبب.

     ⚠️🆕 (إصلاح تعليق متبادل — Deadlock Fix): الحجب هنا بيقتصر بس على
     وجود طرف "pending" — قبل كده كان فيه شرط إضافي بيحجب لو "فيه طرف
     تاني اتعين له صيدلية بس لسه ما وصلش لـ(خرج للتوصيل)"، وده كان بيعمل
     تعليق متبادل: طلب A بيستنى B يوصل، وB بيستنى A يوصل، وكلاهما محجوب
     للأبد ومحدش يقدر يتحرك. بمجرد ما كل الأطراف اتعينت (مفيش pending)،
     كل صيدلية المفروض تقدر تدوس "خرج للتوصيل" في وقتها المستقل — رسالة
     الشحن الموحّدة (trySendCombinedShippingWebhook في orders.js) هي
     أصلاً المسؤولة عن الانتظار لحد ما *كل* الأطراف المُعيّنة توصل فعليًا
     لـ"خرج للتوصيل" قبل ما تتبعت لشركة الشحن، فمفيش داعي لحجب الزرار
     نفسه — الحجب هنا كان تكرار غير صحيح لنفس الحماية وسبب القفلة.
     نفس الإصلاح اتعمل في orders.js -> computeChainShippingBlock. */
  ...
  App.store = {
    ...
    /* ============================================================
       ...
       - غير كده (كل الأطراف اتعينت — بغض النظر هل وصلوا فعليًا
         لـ"خرج للتوصيل" ولا لسه شغالين على خطواتهم) → { blocked: false }،
         عشان كل صيدلية تقدر تدوس زرارها في وقتها المستقل. رسالة الشحن
         الموحّدة (الباك إند) هي اللي بتستنى لحد ما كل الأطراف توصل
         فعليًا لـ"خرج للتوصيل" قبل ما تتبعت — فمفيش داعي نحجب الزرار
         نفسه، وده كان بيعمل تعليق متبادل (اتنين مستنيين بعض للأبد).
    ...
    /* ============================================================
       ...
       — 🆕🆕 (حجب "خرج للتوصيل" لحد ما السلسلة تتحسم) قبل تنفيذ خطوة
         ...
         طرف تاني من نفس سلسلة التنفيذ الجزئي لسه "pending" — لو فيه،
         منعملش أي تحديث محلي أو نداء للباك إند خالص، وبنرجّع
